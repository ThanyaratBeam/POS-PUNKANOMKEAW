...
function loadDB() {
  try {
    ...
    if (saved.sales)         saved.sales         = saved.sales.map(s => ({ ...s, time: new Date(s.time) }));
    if (saved.notifications) saved.notifications = saved.notifications.map(n => ({ ...n, time: new Date(n.time) }));
    if (saved.withdrawals)   saved.withdrawals   = saved.withdrawals.map(w => ({ ...w, time: new Date(w.time) }));
